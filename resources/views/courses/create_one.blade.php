...
@section('content')
<h1>
    {{ $communities[auth()->user()->code] }} 單筆新增課程表
</h1>
<br>
<br>
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">單筆新增課程表</li>
    </ol>
  </nav>
<form action="{{ route('courses.store_one') }}" method="post" id="this_form">
    @csrf
    <div class="mb-3 w-25">
        <label for="year" class="form-label"><span class="text-danger">*</span>年度</label>
        <input type="text" class="form-control rq" id="year" name="year" required onclick="change_button2()" value="{{ $course_season->year }}" readonly>
    </div> 

    <?php 
      $checked1 = ($course_season->season==1)?"checked":null;
      $checked2 = ($course_season->season==2)?"checked":null;
    ?>
    <div class="mb-3">
    ...
    <div class="mb-3 w-25">
        ...
        <input type="date" class="form-control rq" id="start_date" name="start_date" required onclick="change_button2()" value="{{ $course_season->start_date }}" readonly>
    </div>
    <input type="hidden" name="course_season_id" value="{{ $course_season->id }}">
    
    <div class="mb-3 w-50">
      <label for="type" class="form-label"><span class="text-danger">*</span>課程類型</label>
      <input type="text" class="form-control rq" id="type" name="type" required onclick="change_button2()">
    </div>
    ...
    <div class="mb-3 w-25">
      ...
      <input type="text" class="form-control rq" id="place" name="place" required onclick="change_button2()">
    </div>

    <div class="mb-3 w-25">
      <label for="hour" class="form-label"><span class="text-danger">*</span>學分</label>
      <input type="text" class="form-control rq" id="hour" name="hour" required onclick="change_button2()">
    </div>

    <div class="mb-3 w-25">
      <label for="teacher" class="form-label"><span class="text-danger">*</span>授課教師</label>
      <input type="text" class="form-control rq" id="teacher" name="teacher" required onclick="change_button2()">
    </div>

    <div class="mb-3 w-25">
      <label for="time" class="form-label"><span class="text-danger">*</span>上課時間</label>
      <input type="text" class="form-control rq" id="time" name="time" required onclick="change_button2()">
    </div>

    <div class="mb-3 w-25">
      <label for="students" class="form-label">學員總數</label>
      <input type="text" class="form-control" id="students" name="students" onclick="change_button2()">
    </div>
    ...
    <div class="mb-3 w-25">
      ...
      <select id="situation" name="situadtion" class="form-control">
        ...
        <option value="1">開課成功</option>
        <option value="2">開課不成功</option>
      </select>      
    </div>
