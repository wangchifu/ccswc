...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">修改本機使用者</li>
    </ol>
  </nav>
  <form action="{{ route('users.update',$user->id) }}" method="post" id="this_form">
    @csrf
    @method('patch')
    <div class="mb-3">
        ...
        <select class="form-select rq" required id="code" name="code" onchange="check_admin()" onclick="change_button2()">
            ...
            @if($user->code=="079999")
              <option value="079999" selected>教育處人員</option>
            @else
    ...
    <div class="form-check">
      @if($user->admin)
        <input class="form-check-input" type="checkbox" value="1" name="admin" id="admin" checked>
      @else
        @if($user->code == "079999")
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control rq" id="name" name="name" required onclick="change_button2()" value={{ $user->name }}>
    </div>
