...
<nav aria-label="breadcrumb">
  ...
  <form action="{{ route('posts.store') }}" method="post" id="this_form" enctype="multipart/form-data">
    ...
    <?php
      $type_check1 = ($post->type==1)?"checked":null;
      $type_check2 = ($post->type==2)?"checked":null;
    ?>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control rq" id="username" name="title" value="{{ $post->title }}" required onclick="change_button2()">
    </div>

    <div class="mb-3">
      <label for="content" class="form-label"><span class="text-danger">*</span>公告內容</label>
      <textarea class="form-control" id="content" name="content" rows="3" onclick="change_button2()">{{ $post->content }}</textarea>
    </div>
