...
@section('content')
<h1>
    {{ $communities[auth()->user()->code] }} 單筆新增行政人員名冊
</h1>
<br>
<br>
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">單筆新增行政人員名冊</li>
    </ol>
  </nav>
<form action="{{ route('staffs.store_one') }}" method="post" id="this_form">
    @csrf
    <div class="mb-3 w-25">
        <label for="year" class="form-label"><span class="text-danger">*</span>年度</label>
        <input type="text" class="form-control rq" id="year" name="year" required onclick="change_button2()" value="{{ $staff_season->year }}" readonly>
    </div> 

    <?php 
      $checked1 = ($staff_season->season==1)?"checked":null;
      $checked2 = ($staff_season->season==2)?"checked":null;
    ?>
    <div class="mb-3">
    ...
    <input type="hidden" name="staff_season_id" value="{{ $staff_season->id }}">
